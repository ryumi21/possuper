...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="mb-1 fw-bold text-dark" style="font-size: 1.5rem;">Edit Product</h3>
            <p class="text-muted mb-0" style="font-size: 0.85rem;">Update product details</p>
        </div>
        <div>
    ...
    <div class="card border-0 shadow-sm" style="border-radius: 10px;">
        <div class="card-body p-4">
            <form action="{{ route('products.update', $product->Oid) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    <div class="col-md-6">
                        <label for="Code" class="form-label fw-medium text-dark" style="font-size: 0.9rem;">Product Code *</label>
                        <input type="text" class="form-control bg-white border" id="Code" name="Code" required value="{{ old('Code', $product->Code) }}">
                    </div>
                    <div class="col-md-6">
                        <label for="Name" class="form-label fw-medium text-dark" style="font-size: 0.9rem;">Product Name *</label>
                        <input type="text" class="form-control bg-white border" id="Name" name="Name" required value="{{ old('Name', $product->Name) }}">
                    </div>
                    
                    <div class="col-md-6">
                        <label for="Type" class="form-label fw-medium text-dark" style="font-size: 0.9rem;">Product Type</label>
                        <input type="text" class="form-control bg-white border" id="Type" name="Type" value="{{ old('Type', $product->Type) }}">
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control bg-white border" id="Price" name="Price" step="1" required value="{{ old('Price', $product->Price) }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select bg-white border" id="IsStock" name="IsStock" required>
                            <option value="1" {{ old('IsStock', $product->IsStock) == '1' ? 'selected' : '' }}>In Stock</option>
                            <option value="0" {{ old('IsStock', $product->IsStock) == '0' ? 'selected' : '' }}>Out of Stock</option>
                        </select>
                    </div>
                    <div class="col-md-6">
